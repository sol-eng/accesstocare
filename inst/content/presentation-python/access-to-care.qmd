...
print(top_underserved.to_pandas().to_markdown(index=False))
```
...
(
    ggplot(state_polygons_pd, aes(x='x', y='y', fill='pred_status')) +
    geom_polygon(aes(group='group'), color='#cccccc', size=0.3) +
    geom_text(
    ...
    labs(
        title=f'{state_name} - Hospital Access by County',
        fill='Status'
    ) +
    theme_void() +
    theme(
